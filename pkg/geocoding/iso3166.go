...
// alpha2ToAlpha3 ISO 3166-1 Alpha-2 到 Alpha-3 标准国家/地区代码映射表
...
// alpha3ToAlpha2 反向索引表
var alpha3ToAlpha2 map[string]string

func init() {
	alpha3ToAlpha2 = make(map[string]string, len(alpha2ToAlpha3))
	for a2, a3 := range alpha2ToAlpha3 {
		alpha3ToAlpha2[a3] = a2
	}
}

// ToAlpha3 将 ISO 3166-1 Alpha-2 二字国家代码转换为 Alpha-3 三字代码（如 CN -> CHN, US -> USA, JP -> JPN）。
// 若已经是 3 位或无法匹配，则安全返回规范化大写字符串。
func ToAlpha3(alpha2 string) string {
	code := strings.ToUpper(strings.TrimSpace(alpha2))
	if len(code) == 3 {
		return code
	}
...
// ToAlpha2 将 ISO 3166-1 Alpha-3 三字国家代码转换为 Alpha-2 二字代码（如 CHN -> CN, USA -> US, JPN -> JP）。
// 若已经是 2 位或无法匹配，则安全返回规范化大写字符串。
func ToAlpha2(alpha3 string) string {
	code := strings.ToUpper(strings.TrimSpace(alpha3))
	if len(code) == 2 {
		return code
	}
	if a2, ok := alpha3ToAlpha2[code]; ok {
		return a2
	}
	return code
}
